...
const vm = new Vue({
  ...
  async created() {
    ...
    this.addNotification(
      `Keypair Generated - ${this.getKeySnippet(this.originPublicKey)}`
    );
    ...
    async sendMessage() {
      ...
      if (this.destinationPublicKey) {
        ...
        const messageParts = await Promise.all(
          this.sliceIntoChunks(message.get("text"), 245).map(async (part) => {
            return await this.getWebWorkerResponse("encrypt", [
              part,
              this.destinationPublicKey,
            ]);
          })
        );
        const encryptedMsg = message.set("text", messageParts);
